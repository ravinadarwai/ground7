...
$userid = $_SESSION['username']; // Get username from session
...
// Get the turf ID from the URL (e.g., http://localhost/turf_zeeshan/html/coach-detail.php?id=26)
$turfId = isset($_GET['id']) ? $_GET['id'] : null;
...
// Query to fetch the turf owner's information based on the turfId
$query = "SELECT * FROM turf_owners WHERE id = ?";
$stmt = $conn->prepare($query);
$stmt->execute([$turfId]);
$coach = $stmt->fetch(PDO::FETCH_ASSOC);

// Query to fetch games for the selected turf owner
...
// Check if turf owner data is fetched successfully
if ($coach) {
    echo '<div class="dull-bg corner-radius-10 coach-info d-md-flex justify-content-start align-items-start">';
    echo '<div class="profile-pic">';
    echo '<a href="javascript:void(0);"><img alt="User" class="corner-radius-10" src="../admin/' . htmlspecialchars($coach['image']) . '"></a>';
    echo '</div>';
    echo '<div class="info w-100">';
    echo '<h3 class="d-flex align-items-center justify-content-start mb-0">' . htmlspecialchars($coach['turf_name']) . '</h3>';
    ...
    echo '<li class="d-flex align-items-center"><img src="time.png" alt="Icon" style="width: 20px; height: 20px;">Close Time: ' . htmlspecialchars($coach['close_time']) . '</li>';
    echo '</ul>';
    ...
    try {
        // Fetch games available for the selected turf owner
        $stmtGames->execute([$turfId]);
        $games = $stmtGames->fetchAll(PDO::FETCH_ASSOC);

        if (!empty($games)): ?>
            <select class="form-select" id="game-select" aria-label="Select Game">
                <option value="" selected>Select Game</option>
                <?php foreach ($games as $game): ?>
                    <option value="<?php echo htmlspecialchars($game['game_name'] . ',' . $game['price_per_person']); ?>">
                        <?php echo htmlspecialchars($game['game_name']); ?> - ₹<?php echo htmlspecialchars($game['price_per_person']); ?>
                    </option>
                <?php endforeach; ?>
            </select>
        <?php else: ?>
            <p>No games available for this turf owner.</p>
        <?php endif;
    } catch (PDOException $e) {
        echo "Error fetching games: " . $e->getMessage();
    }

    echo '<button id="book-now-btn" class="btn btn-secondary d-inline-flex justify-content-center align-items-center mt-5">
            <i class="feather-calendar"></i> Book Now
          </button>';
    ...
    echo '<p>No turf owner data found.</p>';
}
?>
<script>
    document.getElementById('book-now-btn').addEventListener('click', function () {
        ...
        if (selectedOption) {
            ...
            const [gameName, gamePrice] = selectedOption.split(',');

            // Redirect to book-slot.php with query parameters
            window.location.href = `book-slot.php?id=<?php echo htmlspecialchars($coach['id']); ?>&game_name=${encodeURIComponent(gameName)}&game_price=${encodeURIComponent(gamePrice)}`;
        } else {
            alert('Please select a game before booking!');
        }
    });
</script>
                        ...
                        <div class="venue-options white-bg mb-4">
                            <ul class="clearfix">
                                ...
                                <li><a href="javascript:void(0);" onclick="scrollToSection('location')">Locations</a>
                                </li>
                            </ul>
                        </div>
    ...
    try {
        ...
        // Fetch images from the database for the specific turf owner
        $sql = "SELECT image FROM gallery WHERE user_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$turfId]);  // Use $turfId here
        $images = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Fetch games associated with the turf owner
        $sqlGames = "SELECT game_name, game_description, game_image, price_per_person FROM games WHERE turf_owners_id = ?";
        $stmtGames = $conn->prepare($sqlGames);
        $stmtGames->execute([$turfId]);  // Use $turfId here
        $games = $stmtGames->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        // Handle SQL errors
                            ...
                            <div class="accordion-item mb-4" id="games">
                                <h4 class="accordion-header" id="panelsStayOpen-games">
                                    <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#panelsStayOpen-collapseGames" aria-expanded="false"
                                        aria-controls="panelsStayOpen-collapseGames">
                                        Available Games
                                    </button>
                                </h4>
                                <div id="panelsStayOpen-collapseGames" class="accordion-collapse collapse show"
                                                    ...
                                                    <div class="col-md-4 mb-4">
                                                        <div class="card">
                                                            <img class="card-img-top img-fluid rounded"
                                                                ...
                                                                src="../admin/<?php echo htmlspecialchars($game['game_image']); ?>"
                                                                alt="<?php echo htmlspecialchars($game['game_name']); ?>">
                                                            <div class="card-body">
                                                                <h5 class="card-title">
                                                                    <?php echo htmlspecialchars($game['game_name']); ?></h5>
                                                                <p class="card-text">
                                                                    <?php echo htmlspecialchars($game['game_description']); ?>
                                                                </p>
                                                                <p class="card-text"><strong>Price:
                                                                        <?php echo htmlspecialchars($game['price_per_person']); ?>
                                                                        per person</strong></p>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <?php if (!empty($images)): ?>
                                                <?php foreach ($images as $image): ?>
                                                    <div class="gallery-widget-item">
                                                        <a class="corner-radius-10"
                                                            href="../admin/<?php echo htmlspecialchars($image['image']); ?>"
                                                            data-fancybox="gallery2">
                                                            <img class="img-fluid corner-radius-10" alt="Image"
                                                                src="../admin/<?php echo htmlspecialchars($image['image']); ?>">
                                                        </a>
                                                    </div>
                                                <?php endforeach; ?>
                                            <?php else: ?>
                            ...
                            <div class="accordion-item mb-4" id="reviews">
                                <div class="accordion-header" id="panelsStayOpen-reviews">
                                    ...
                                    <a href="javascript:void(0);"
                                        class="btn btn-gradient pull-right write-review add-review"
                                        data-bs-toggle="modal" data-bs-target="#add-review">Write a review</a>
                                </div>
                                <div id="panelsStayOpen-collapseSix" class="accordion-collapse collapse show"
                                    ...
                                    <div class="accordion-body">
                                        ...
                                        <div class="d-flex justify-content-center">
                                            <button type="button"
                                                class="btn btn-load-more d-flex justify-content-center align-items-center">Load
                                                More<i class="feather-plus-square"></i></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="accordion-body">
                                        ...
                                        <?php if (!empty($location['location'])): ?>
                                            <div class="google-maps">
                                                <iframe src="<?php echo htmlspecialchars($location['location']); ?>"
                                ...
                                <?php foreach ($result as $event): ?>
                                    <li>
                                        <div class="post-thumb">
                                            <a
                                                href="event_registration.php?id=<?php echo htmlspecialchars($event['id']); ?>">
                                                <img class="img-fluid"
                                                    src="../admin/<?php echo htmlspecialchars($event['image']); ?>"
                                                    alt="Post">
                                            </a>
                                        </div>
                                        <div class="post-info">
                                    ...
                                    <div class="input-space review">
                                        ...
                                        <div class="star-rating">
                                            ...
                                            <label for="star-5" title="5 stars"><i
                                                    class="active fa fa-star"></i></label>
