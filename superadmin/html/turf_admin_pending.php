...
<body>
    ...
    <div id="main-wrapper" data-layout="vertical" data-navbarbg="skin5" data-sidebartype="full"
        ...
        $stmt = $conn->prepare("SELECT id, owner_name, turf_name, turf_location FROM turf_owners WHERE is_verified = 0");
        $stmt->execute();
        $turfOwners = $stmt->fetchAll(PDO::FETCH_ASSOC);

        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            // Get the turf owner ID from the form submission
            $owner_id = $_POST['owner_id'];

            // Update the turf owner's verification status
            $stmt = $conn->prepare("UPDATE turf_owners SET is_verified = 1 WHERE id = :owner_id");
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if (strlen($location) > 5) { ?>
                                <a href="javascript:void(0);" class="see-more-btn" onclick="toggleLocation(this)">See more</a>
                            <?php } ?>
                        </td>
                        <td>
